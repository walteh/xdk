...
let swiftErr = Git(
	modules: ["Err", "LogEvent", "LogDistributor"],
	version: "0.12.0",
	url: "https://github.com/walteh/swift-err.git"
)
.apply(god)
...
let swiftXid = Git(
	module: "xid",
	version: "0.2.1",
	url: "https://github.com/uatuko/swift-xid.git"
)
.apply(god)
let ecdsa = Git(
	module: "MicroDeterministicECDSA",
	version: "0.9.0",
	url: "https://github.com/walteh/micro-deterministic-ecdsa.git"
).apply(god)
let swiftContext = Git(
...
let awssso = Local(name: "AWSSSO").with(deps: [
	x, awssdk.child(module: "AWSSSO"), awssdk.child(module: "AWSSSOOIDC"), swiftErr,
]).apply(god)
...
class Git {
	...
	init(modules: [String], version: String, url: String) {
		self.name = url.split(separator: "/").last!.replacingOccurrences(of: ".git", with: "")
		self.modules = modules

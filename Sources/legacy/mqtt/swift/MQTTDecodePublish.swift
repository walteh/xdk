...
public class MQTTDecodePublish: NSObject {
	...
	public func decodePublish(fixedHeader: UInt8, publishData: [UInt8]) {
		...
		printDebug("topic = \(self.topic)")

		guard let recQos = MQTTQoS(rawValue: (fixedHeader & 0b0000_0110) >> 1) else {
			return
		}
		...
		if recQos == .qos1 || recQos == .qos2 {
			...
			self.packetIdentifier = UInt16(IdentifierResult!.res)
		...
		if protocolVersion == "5.0" {
			...
			while dataIndex < occupyIndex + (self.propertyLength ?? 0) {
				...
				// 3.3.2.3.2 Payload Format Indicator
				case MQTTPropertyName.payloadFormatIndicator.rawValue:
					if publishData[dataIndex] & 0b0000_0001 > 0 {
						self.payloadFormatIndicator = .utf8
					} else {
						self.payloadFormatIndicator = .unspecified
					}
					dataIndex += 1

				// 3.3.2.3.3 Message Expiry Interval
				case MQTTPropertyName.willExpiryInterval.rawValue:
					let comRes = integerCompute(data: publishData, formatType: formatInt.formatUint32.rawValue, offset: dataIndex)
					self.messageExpiryInterval = UInt32(comRes!.res)
					dataIndex = comRes!.newOffset

				// 3.3.2.3.4 Topic Alias
				case MQTTPropertyName.topicAlias.rawValue:
					let comRes = integerCompute(data: publishData, formatType: formatInt.formatUint16.rawValue, offset: dataIndex)
					self.topicAlias = UInt16(comRes!.res)
					dataIndex = comRes!.newOffset

				// 3.3.2.3.5 Response Topic
				case MQTTPropertyName.responseTopic.rawValue:
					guard let result = unsignedByteToString(data: publishData, offset: dataIndex) else {
						break
					}
					self.responseTopic = result.resStr
					dataIndex = result.newOffset

				// 3.3.2.3.6 Correlation Data
				case MQTTPropertyName.correlationData.rawValue:
					guard let result = unsignedByteToBinary(data: publishData, offset: dataIndex) else {
						break
					}
					self.correlationData = result.resStr
					dataIndex = result.newOffset
				...
				// 3.3.2.3.8 Subscription Identifier
				case MQTTPropertyName.subscriptionIdentifier.rawValue:
					let valRes = decodeVariableByteInteger(data: publishData, offset: dataIndex)
					self.subscriptionIdentifier = valRes.res
					dataIndex = valRes.newOffset

				// 3.3.2.3.9 Content Type
				case MQTTPropertyName.contentType.rawValue:
					guard let valRes = unsignedByteToString(data: publishData, offset: dataIndex) else {
						break
					}
					self.contentType = valRes.resStr
					dataIndex = valRes.newOffset

...
@objc public enum MQTTQoS: UInt8, CustomStringConvertible {
...
extension Frame {
	...
	func bytes(version: String) -> [UInt8] {
		if version == "5.0" {
			...
			printDebug("remainingLen(len: len) \(self.remainingLen(len: len5))")
			printDebug("variableHeader \(variableHeader5)")
			printDebug("properties \(properties)")
			printDebug("payload \(payload5)")
			printDebug("=============================================================")

			return [packetFixedHeaderType] + self.remainingLen(len: len5) + variableHeader5 + properties + payload5
		} else {
			...
			printDebug("packetFixedHeaderType \(packetFixedHeaderType)")
			printDebug("remainingLen(len: len) \(self.remainingLen(len: len))")
			printDebug("variableHeader \(variableHeader)")
			printDebug("payload \(payload)")
			printDebug("=============================================================")

			return [packetFixedHeaderType] + self.remainingLen(len: len) + variableHeader + payload
		}
	}

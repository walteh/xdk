...
public extension big.UInt {
	...
	/// - Complexity: O(count)
	internal func subtractingWordReportingOverflow(_ word: Word, shiftedBy shift: Int = 0) -> (partialValue: big.UInt, overflow: Bool) {
		var result = self
	...
	internal mutating func subtractWord(_ word: Word, shiftedBy shift: Int = 0) {
		let overflow = self.subtractWordReportingOverflow(word, shiftedBy: shift)
	...
	/// - Requires: self >= d * 2^shift
	/// - Complexity: O(count)
	internal func subtractingWord(_ word: Word, shiftedBy shift: Int = 0) -> big.UInt {
		var result = self
	...
	/// - Complexity: O(count)
	mutating func subtractReportingOverflow(_ b: big.UInt, shiftedBy shift: Int = 0) -> Bool {
		precondition(shift >= 0)
		var carry = false
		var bi = 0
		let bc = b.count
	...
	/// - Complexity: O(count)
	func subtractingReportingOverflow(_ other: big.UInt, shiftedBy shift: Int) -> (partialValue: big.UInt, overflow: Bool) {
		var result = self
	...
	/// - Complexity: O(count)
	func subtractingReportingOverflow(_ other: big.UInt) -> (partialValue: big.UInt, overflow: Bool) {
	...
	/// - Complexity: O(count)
	mutating func subtract(_ other: big.UInt, shiftedBy shift: Int = 0) {
		let overflow = self.subtractReportingOverflow(other, shiftedBy: shift)
		precondition(!overflow)
	}
	...
	/// - Complexity: O(count)
	func subtracting(_ other: big.UInt, shiftedBy shift: Int = 0) -> big.UInt {
		var result = self
	...
	static func - (a: big.UInt, b: big.UInt) -> big.UInt {

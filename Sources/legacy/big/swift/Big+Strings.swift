...
extension big.UInt {
	...
	public init?(_ text: some StringProtocol, radix: Int = 10) {
		...
		let (charsPerWord, power) = big.UInt.charsPerWord(forRadix: radix)
...
public extension big.Int {
	...
	init?(_ text: some StringProtocol, radix: Int = 10) {
		var magnitude: big.UInt?
		var sign: Sign = .plus
		if text.first == "-" {
			sign = .minus
			let text = text.dropFirst()
			magnitude = big.UInt(text, radix: radix)
		} else if text.first == "+" {
			let text = text.dropFirst()
			magnitude = big.UInt(text, radix: radix)
		} else {
...
public extension String {
	...
	init(_ v: big.UInt) { self.init(v, radix: 10, uppercase: false) }
	...
	init(_ v: big.UInt, radix: Int, uppercase: Bool = false) {
		precondition(radix > 1)
		let (charsPerWord, power) = big.UInt.charsPerWord(forRadix: radix)
	...
	/// - Requires: radix > 1 && radix <= 36
	/// - Complexity: O(count) when radix is a power of two; otherwise O(count^2).
	init(_ value: big.Int, radix: Int = 10, uppercase: Bool = false) {
...
extension big.Int: ExpressibleByStringLiteral {
	/// Initialize a new big integer from a Unicode scalar.
	/// The scalar must represent a decimal digit.
	public init(unicodeScalarLiteral value: UnicodeScalar) {
		self = big.Int(String(value), radix: 10)!
	}

	/// Initialize a new big integer from an extended grapheme cluster.
	/// The cluster must consist of a decimal digit.
	public init(extendedGraphemeClusterLiteral value: String) {
		self = big.Int(value, radix: 10)!
	}
...
extension big.UInt: CustomStringConvertible {
...
extension big.Int: CustomStringConvertible {
	/// Return the decimal representation of this integer.
	public var description: String {
		return String(self, radix: 10)
	}
...
extension big.UInt: CustomPlaygroundDisplayConvertible {
...
extension big.Int: CustomPlaygroundDisplayConvertible {
	/// Return the playground quick look representation of this integer.
	public var playgroundDescription: Any {
		let text = String(self)

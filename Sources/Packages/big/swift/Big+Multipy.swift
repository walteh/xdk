...
public extension big.UInt {
	...
	///
	/// - Complexity: O(count)
	func multiplied(byWord y: Word) -> big.UInt {
		var r = self
		r.multiply(byWord: y)
		return r
	}
	...
	/// - Complexity: O(count)
	mutating func multiplyAndAdd(_ x: big.UInt, _ y: Word, shiftedBy shift: Int = 0) {
	...
	///   `big.UInt.directMultiplicationLimit` words.
	/// - Complexity: O(n^log2(3))
	func multiplied(by y: big.UInt) -> big.UInt {
	...
	static func * (x: big.UInt, y: big.UInt) -> big.UInt {
		let xc = x.count
		let yc = y.count
		if xc == 0 { return big.UInt() }
		if yc == 0 { return big.UInt() }
		if yc == 1 { return x.multiplied(byWord: y[0]) }
		if xc == 1 { return y.multiplied(byWord: x[0]) }

		if Swift.min(xc, yc) <= big.UInt.directMultiplicationLimit {
			// Long multiplication.
			let left = (xc < yc ? y : x)
			let right = (xc < yc ? x : y)
			var result = big.UInt()

...
extension FixedWidthInteger {
	...
	private var high: Self {
		return self &>> self.halfShift
	}

	private var low: Self {
		let mask: Self = 1 &<< self.halfShift - 1
		return self & mask
	}
...
public extension big.Int {
	...
	func quotientAndRemainder(dividingBy y: big.Int) -> (quotient: big.Int, remainder: big.Int) {
		var a = self.magnitude
		var b = y.magnitude
		big.UInt.divide(&a, by: &b)
		return (big.Int(sign: self.sign == y.sign ? .plus : .minus, magnitude: a),
		        big.Int(sign: self.sign, magnitude: b))
	}

	/// Divide `a` by `b` and return the quotient. Traps if `b` is zero.
	static func / (a: big.Int, b: big.Int) -> big.Int {
		return big.Int(sign: a.sign == b.sign ? .plus : .minus, magnitude: a.magnitude / b.magnitude)
	}

	/// Divide `a` by `b` and return the remainder. The result has the same sign as `a`.
	static func % (a: big.Int, b: big.Int) -> big.Int {
		return big.Int(sign: a.sign, magnitude: a.magnitude % b.magnitude)
	}

	/// Return the result of `a` mod `b`. The result is always a nonnegative integer that is less than the absolute value of `b`.
	func modulus(_ mod: big.Int) -> big.Int {
...
public extension big.Int {
	...
	static func /= (a: inout big.Int, b: big.Int) { a = a / b }
...
public extension big.UInt {
	...
	internal func quotientAndRemainder(dividingByWord y: Word) -> (quotient: big.UInt, remainder: Word) {
		var div = self
	...
	internal static func divide(_ x: inout big.UInt, by y: inout big.UInt) {
		...
		if y.count == 1 {
			...
			y = big.UInt(x.divide(byWord: y[0]))
			return
		}
	...
	internal mutating func formRemainder(dividingBy y: big.UInt, normalizedBy shift: Int) {
		precondition(!y.isZero)
		assert(y.leadingZeroBitCount == 0)
		if y.count == 1 {
			let remainder = self.divide(byWord: y[0] >> shift)
			self.load(big.UInt(remainder))
			return
		}
		self <<= shift
		if self >= y {
	...
	/// - Returns: `(quotient, remainder)` where `quotient = floor(self/y)`, `remainder = self - quotient * y`
	/// - Complexity: O(count^2)
	func quotientAndRemainder(dividingBy y: big.UInt) -> (quotient: big.UInt, remainder: big.UInt) {
		var x = self
		var y = y
		big.UInt.divide(&x, by: &y)
		return (x, y)
	}
	...
	static func / (x: big.UInt, y: big.UInt) -> big.UInt {
	...
	///
	/// - Note: Use `divided(by:)` if you also need the remainder.
	static func /= (x: inout big.UInt, y: big.UInt) {
		var y = y
		big.UInt.divide(&x, by: &y)
	}

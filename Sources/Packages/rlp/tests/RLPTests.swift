...
extension RLPTests {
	...
	func testEncodeLengthGreaterThan55() {
		XCTAssertEqual(RLP.encodeLength(0x38, offset: 0x80).hexEncodedString(), Data([0xB8, 0x38]).hexEncodedString())
		XCTAssertEqual(RLP.encodeLength(0x0400, offset: 0x80), Data([0xB9, 0x04, 0x0]))
		XCTAssertEqual(RLP.encodeLength(0x11170, offset: 0x80), Data([0xBA, 0x01, 0x11, 0x70]))
	}
}
...
extension RLPTests {
	...
	func testEncodeStringGreaterThan55Bytes() {
		...
		let expectedBytes: [UInt8] = [0xB9, 0x01, 0x90] + [UInt8](repeating: 0x61, count: 400)

...
@err func invalidateRoleCredentials(
	_ storage: some StorageAPI,
	role: RoleInfo
) -> Result<Bool, Error> {
	...
	guard
		let _ = XDK.Delete(
			...
			differentiator: role.uniqueID + XDKAWSSSO_KEYCHAIN_VERSION
		).get()
	else {
...
@err func invalidateAndGetRoleCredentialsUsing(
	...
	return await getRoleCredentialsUsing(
		sso: client,
		storage: storage,
...
@err public func getRoleCredentialsUsing(
	...
	guard let curr = XDK.Read(using: storage, RoleCredentials.self, differentiator: myid).get()
	else {
	...
	if let curr {
		if curr.expiresIn() > 60 * 5, curr.expiresIn() < 60 * 60 * 24 * 7 {
			log(.debug).info("creds", curr.accessKeyID).info("account", role.accountID).info(
				"role",
				role.roleName
			)
			...
			log(.debug).info("expires_in", any: curr.expiresIn().seconds()).info(
				"expires_at",
				curr.expiresAt
			)
	...
	guard
		let creds = await client.getRoleCredentials(
			input: .init(
	...
	guard let _ = XDK.Write(using: storage, rcreds, overwrite: true, differentiator: myid).get()
	else {
		return .failure(x.error("error writing role creds to keychain", root: err))
	}

	log(.debug).info("creds", rcreds.accessKeyID).info("account", role.accountID).info(
		"uniqueID",
		role.uniqueID
	).info("role", role.roleName).info("expiresAt", rcreds.expiresAt).send("writing creds to cache")
...
@err func fetchCachedSignInToken(
	...
	guard
		let curr = XDK.Read(using: storage, RoleCredentialsSignInToken.self, differentiator: myid)
			.get()
	else {
...
@err func fetchSignInToken(
	with credentials: RoleCredentials,
	retryNumber: Int = 0
	...
	guard
		let (data, response) = await Result({ try await URLSession.shared.data(for: request) })
			.get()
	else {
	...
	if httpResponse.statusCode == 400 {
		if retryNumber < 5, retryNumber >= 0 {
			log(.debug).info("account", credentials.role.accountID).info("count", any: retryNumber)
				.send("retrying fetchSignInToken")
	...
	if httpResponse.statusCode != 200 {
		...
		let lastfirst =
			String(data: data, encoding: .utf8)!.prefix(10) + "..."
			+ String(data: data, encoding: .utf8)!.suffix(10).replacingOccurrences(
				of: "\n",
				with: ""
			)

		return .failure(
			x.error("unexpected error code: \(httpResponse.statusCode)").info("body", lastfirst)
				.info("url", request.url?.absoluteString ?? "none")
		)
	} else {
		log(.debug).info("request_url", request.url?.absoluteString ?? "none").send(
			"success on fetchSignInToken"
		)
	}

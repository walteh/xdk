...
struct FrameUnsubscribe: Frame {
	var packetFixedHeaderType: UInt8 = .init(FrameType.unsubscribe.rawValue + 2)
...
extension FrameUnsubscribe: CustomStringConvertible {
	var description: String {
		...
		if protocolVersion == "5.0" {
			...
			return "UNSUBSCRIBE(id: \(String(describing: self.msgid)), topics: \(String(describing: self.topics)))"
		}
	}
}

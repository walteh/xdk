...
struct FrameSubAck: Frame {
	...
	init(msgid: UInt16, grantedQos: [MQTTQoS]) {
		self.msgid = msgid
...
extension FrameSubAck {
	...
	func variableHeader5() -> [UInt8] { return self.msgid.hlBytes }
	...
	init?(packetFixedHeaderType: UInt8, bytes: [UInt8]) {
		...
		if protocolVersion == "5.0" {
			...
			for i in 3 ..< bytes.count {
				guard let qos = MQTTQoS(rawValue: bytes[i]) else {
					return nil
				}
				self.grantedQos.append(qos)
			}

			self.reasonCodes = [MQTTSUBACKReasonCode]()
			for i in 3 ..< bytes.count {
				guard let qos = MQTTSUBACKReasonCode(rawValue: bytes[i]) else {
					return nil
				}
